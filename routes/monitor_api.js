      ...
      .then(conn => {
        conn.query('SELECT * FROM guardians WHERE name = ?', req.params.guardian_id)
            .then(id => {
              ...
              var auraPromise = conn.query('SELECT * FROM aura_data WHERE guardian = ? ORDER BY time DESC LIMIT 1', req.params.guardian_id);
              auraPromise
                  .then(rows => {
                    if (rows.length > 0) {
              ...
              var camsPromise = conn.query('SELECT * FROM cams_data WHERE guardian = ? ORDER BY time DESC LIMIT 1', req.params.guardian_id);
              camsPromise
                  .then(rows => {
                    if (rows.length > 0) {
